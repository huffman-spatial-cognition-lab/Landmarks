...
public class LM_ExpStartup : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        if (!Directory.Exists(appDir + "/" + config.experiment)) Directory.CreateDirectory(appDir + "/" + config.experiment);
        Debug.Log("------------------------------ " + appDir + "/" + config.experiment);

        if (id != 0 | guiElements.subID == null)
        {
            // Set a default ID if need be 
            Debug.LogError("No field for providing a subject id manually; automatically generating id starting at 101");
            autoID = 101;
            ...
            guiElements.subID.text = autoID.ToString();
            id = autoID;
